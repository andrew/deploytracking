...
Capistrano::Configuration.instance(:must_exist).load do
  ...
  namespace :deploy do
    ...
    task :track, :except => { :no_release => true } do
      ...
      data = "deploy[github_username]=#{github_username}&deploy[environment]=#{rails_env}&deploy[revision]=#{current_revision}&deploy[repository]=#{repository}&api_key=#{deploy_tracking_api_key}&source=capistrano"
